...
export const prismaGetFilteredUsers = async (filteres) => {
    try {
        ...
        filtersArr.forEach((filter) => {
            where = Object.assign(Object.assign({}, where), { [filter[0]]: filter[1] });
        });
...
export const prismaGetUsersBySource = async (sourceId) => {
    try {
...
export const prismaSaveUser = async (user) => {
    try {
...
export const prismaUpdateUser = async (user) => {
    try {
